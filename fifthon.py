...
async def join_channel():
    try:
        await fifthon(JoinChannelRequest("cloversource"))
...
async def _(event):
    ...
    await event.edit(f'''
**★ 𝙒𝙚𝙡𝙘𝙤𝙢𝙚 ★ 𝙎𝙤𝙪𝙧𝙘𝙚 𝘾𝙡𝙤𝙫𝙚𝙧 ★

• 𝙑𝙚𝙧𝙨𝙞𝙤𝙣 : 7.0
• 𝙔𝙤𝙧 𝙞𝙙 : {event.sender_id} 
• 𝙙𝙖𝙩𝙚 : {m9zpi} 

★𝘾𝙝𝙖𝙣𝙣𝙚𝙡 @CloverSource 𝘿𝙚𝙫 @x_k_5**
''')
...
@fifthon.on(events.NewMessage(outgoing=True, pattern=r"\.قناة السورس"))
async def _(event):
    start = datetime.datetime.now()
...
@fifthon.on(events.NewMessage(outgoing=False, pattern='/tnt'))
